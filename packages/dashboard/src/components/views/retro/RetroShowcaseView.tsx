...
export function RetroShowcaseView({ settings }: Props): React.ReactElement {
  ...
  useEffect(() => {
    ...
    async function fetchData(): Promise<void> {
      try {
        const res = await fetch(withActiveProfile('/api/retro/recent-games?count=10'));
        ...
        const infoRes = await fetch(withActiveProfile(`/api/retro/game-info?gameId=${game.GameID}`));
        ...
        const heroPromise = info.Title ? (async () => {
          try {
            const searchRes = await fetch(withActiveProfile(`/api/sgdb/search?term=${encodeURIComponent(info.Title)}`));
        ...
        const screenshotPromise = info.Title ? (async () => {
          try {
            const igdbRes = await fetch(withActiveProfile(`/api/igdb/screenshots?game=${encodeURIComponent(info.Title)}`));
