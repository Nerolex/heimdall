...
function igdbCredentials(query: { clientId?: string; clientSecret?: string; profile?: string }) {
  const result = loadConfig(query.profile ? resolveProfileConfigPath(query.profile) : resolveConfigPath());
  const fromConfig = result.config?.igdb;
  return {
    clientId: fromConfig?.clientId || query.clientId || '',
    clientSecret: fromConfig?.clientSecret || query.clientSecret || '',
  };
}

export async function igdbRoute(fastify: FastifyInstance): Promise<void> {
  fastify.get<{ Querystring: { clientId?: string; clientSecret?: string; game?: string } }>('/api/igdb/screenshots', async (request) => {
    const { clientId, clientSecret } = igdbCredentials(request.query);
    const game = request.query.game;
    if (!clientId || !clientSecret || !game) return { error: 'Missing clientId, clientSecret, or game' };

    const cacheKey = `igdb:${clientId}:${clientSecret}:${game}`;
